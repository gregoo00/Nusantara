...
const Data = [
  {
    ...
    description:
      "Cimol Bojot AA adalah jajanan khas Bandung yang sedang viral, terutama di kalangan pecinta kuliner pedas. Berbahan dasar tepung tapioka, cimol ini memiliki tekstur kenyal dan tidak kopong, disajikan dengan bumbu tabur yang gurih, pedas, dan aromatik, seperti balado daun jeruk, bawang goreng, atau bon cabe",
  },
  {
    id: 2,
    imgSrc: [WarungNasi],
...
// Data rekomendasi restoran lain
...
const RestaurantPage = () => {
  ...
  const handleCommentSubmit = (e) => {
    ...
    if (comment.trim() && rating > 0) {
      setReviews([...reviews, { rating, comment, id: Date.now() }]);
      setComment("");
      setRating(0);
    }
  };
  ...
  const renderStars = (grade) => {
    ...
    for (let i = 1; i <= 5; i++) {
      if (i <= Math.floor(ratingValue)) { 
        stars.push(<FaStar key={i} className="star-icon star-filled" />);
      } else if (i === Math.ceil(ratingValue) && ratingValue % 1 !== 0) {
        stars.push(<FaStarHalfAlt key={i} className="star-icon star-half" />);
      } else {
        stars.push(<FaRegStar key={i} className="star-icon star-empty" />);
      }
    }
    return stars;
  };
  ...
  const handleNextSlide = () => {
    if (currentImageIndex < restaurant.imgSrc.length - 1) {
      setCurrentImageIndex(currentImageIndex + 1);
    }
  };

  // Filter rekomendasi untuk mengecualikan restoran yang sedang dilihat
      ...
      <section className="restaurant-page">
        <div className="container">
          ...
          <div data-aos="fade-up" className="image-slider">
            ...
            {currentImageIndex > 0 && (
              <button className="slider-arrow left" onClick={handlePrevSlide}>
                <FaArrowLeft />
              </button>
            )}
            {currentImageIndex < restaurant.imgSrc.length - 1 && (
              <button className="slider-arrow right" onClick={handleNextSlide}>
                <FaArrowRight />
              </button>
            )}
          ...
          <div data-aos="fade-up" className="review-section">
              ...
              {[1, 2, 3, 4, 5].map((value) => (
                <FaStar
                  ...
                  className={`star-icon rating-star ${
                    value <= rating ? "star-filled" : "star-empty"
                  }`}
            ...
            <div className="comment-input">
              <textarea
                ...
                rows="4"
                placeholder="Write your review about the restaurant or food..."
                value={comment}
